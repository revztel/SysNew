<?php
// Delay the script execution for 60 second
...
logToFile($logFilePath, "Received callback data in query2_update.php at " . $receivedTimestamp . ":\n" . $captureLogs);
...
// Attempt to extract ResultCode from nested structures if not present at the top level
    ...
    if (!$PaymentGatewayRecord) {
        ...
        if (!$userid) {
            ...
            if (!$plans) {
                ...
                file_put_contents('query2update.log', "Including external script: $file_path\n", FILE_APPEND);
                include_once $file_path;
                file_put_contents('query2update.log', "external script included: $file_path\n", FILE_APPEND);
                ...
                if ($PaymentGatewayRecord->status == 2) {
                    ...
                    if ($existing_recharge) {
                        ...
                        ORM::for_table('tbl_user_recharges')->create(array(
                            ...
                            'time' => $expiry_time,
                            'status' => "on",
                            'method' => $PaymentGatewayRecord->gateway . "-STK_FAILOVER2",
                    ...
                    $existingTransactions = ORM::for_table('tbl_transactions')
                        ->where('invoice', 'STK_FAILOVER2' . $checkout_req_id)
                        ->order_by_desc('id')
                        ->find_many();
                    ...
                    if (count($existingTransactions) > 1) {
                        ...
                        foreach ($existingTransactionsArray as $transaction) {
                            ...
                            if ($transactionToDelete) {
                                ...
                                logToFile($logFilePath, "Deleted duplicate transaction with invoice: " . 'STK_FAILOVER2' . $checkout_req_id . " and ID: " . $transaction['id']);
                            } else {
                                logToFile($logFilePath, "Failed to find transaction with ID: " . $transaction['id']);
                            }
                        }
                    } else {
                        logToFile($logFilePath, "No duplicates found or only one transaction found for invoice: " . 'STK_FAILOVER2' . $checkout_req_id);
                    }

                    if (count($existingTransactions) == 0) {
                        // Insert new record into tbl_transactions
                        ORM::for_table('tbl_transactions')->create(array(
                            'invoice' => 'STK_FAILOVER2' . (ORM::for_table('tbl_transactions')->max('id') + 1), // Incremental QUERY2 code
                            'username' => $uname,
                            'plan_name' => $plan_name,
                        ...
                        logToFile($logFilePath, "Transaction record already exists for invoice: " . 'STK_FAILOVER2' . $checkout_req_id);
                    }
                    ...
                    if (count($secondaryCheckTransactions) > 1) {
                        ...
                        foreach ($secondaryCheckTransactionsArray as $transaction) {
                            ...
                            if ($transactionToDelete) {
                                $transactionToDelete->delete();
                                logToFile($logFilePath, "Deleted duplicate transaction with invoice in secondary check: " . 'STK_FAILOVER2' . $checkout_req_id);
                            } else {
                                logToFile($logFilePath, "Failed to find transaction with ID: " . $transaction['id']);
                            }
                        }
                    ...
                    $PaymentGatewayRecord->gateway_trx_id = 'STK_FAILOVER2' . $checkout_req_id;
