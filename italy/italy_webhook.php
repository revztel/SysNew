 ...
 * }
 *
 * Then processes recharges, uses the specified plan_id, price, and router_id,
...
// Optional or required fields, with defaults if missing
$uname    = trim($event['username']);
$plan_id  = isset($event['plan_id'])  ? (int) $event['plan_id']  : 1;
$price    = isset($event['price'])    ? (float) $event['price']  : 1.0;
$routerID = isset($event['router_id'])? (int) $event['router_id']: 1;

logToFile($logFilePath, "Starting Italy webhook for user=$uname plan_id=$plan_id price=$price router_id=$routerID");
...
// ------------------------------------------------
// 4b) Fetch router name from tbl_routers
// ------------------------------------------------
$router = ORM::for_table('tbl_routers')->find_one($routerID);
if (!$router) {
    logToFile($logFilePath, "Router not found: ID=$routerID");
    http_response_code(200);
    exit;
}
$routerName = $router->name;
logToFile($logFilePath, "Router found => [ID=$routerID, name=$routerName]");
...
try {
    ...
    $recharge->status         = 'on';
    $recharge->method         = 'ItalyWebhook'; // or any label
    // store the router name for reference
    $recharge->routers        = $routerName; 
...
try {
    ...
    $transaction->method         = 'ItalyWebhook'; // or any label
    // store the router name for reference
...
try {
    ...
    // If you have config from tbl_appconfig, fetch it here if needed

    // This is an example call. Make sure the Message class is loaded:
...
// ------------------------------------------------
$file_path = __DIR__ . '/../system/additaly.php'; // Adjust if needed
...
// 11) Done
// ------------------------------------------------
logToFile($logFilePath, "Webhook processing complete for user=$uname with plan_id=$plan_id, router_id=$routerID.");
http_response_code(200);
exit;
