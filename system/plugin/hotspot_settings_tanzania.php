...
function hotspot_settings_tanzania() {
    global $ui, $conn;
    _admin();
    $ui->assign('_title', 'Hotspot Dashboard - Tanzania');
    $admin = Admin::_info();
    $ui->assign('_admin', $admin);

    // Fetch the Tanzania-specific router_id if stored separately
    $stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'router_id_tanzania'");
    $stmt->execute();
    ...
    // Using the same variables as Kenya
    $settingsKeys = [
        'hotspot_title',
        'description',
        'free_trial',
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Update router_id for Tanzania
        $updateRouterIdStmt = $conn->prepare("UPDATE tbl_appconfig SET value = :router_id WHERE setting = 'router_id_tanzania'");
...
EOL;
        ...
        r2(U . "plugin/hotspot_settings_tanzania", 's', "Settings Saved and Uploaded to Router");
    }
    ...
    $ui->display('hotspot_settings_tanzania.tpl');
}
