...
$freeTrialEnabled = ($result && $result['value'] === 'enable');
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // 1) BASIC FORM FIELDS (title, color scheme, FAQ, etc.)
    
        // If user didn't input anything, fallback to the old values.
        ...
        $newFaqHeadline1      = isset($_POST['frequently_asked_questions_headline1']) 
                                    ? trim($_POST['frequently_asked_questions_headline1']) 
                                    : $settings['frequently_asked_questions_headline1'];
        $newFaqHeadline2      = isset($_POST['frequently_asked_questions_headline2']) 
                                    ? trim($_POST['frequently_asked_questions_headline2']) 
                                    : $settings['frequently_asked_questions_headline2'];
        $newFaqHeadline3      = isset($_POST['frequently_asked_questions_headline3']) 
                                    ? trim($_POST['frequently_asked_questions_headline3']) 
                                    : $settings['frequently_asked_questions_headline3'];
        $newFaqAnswer1        = isset($_POST['frequently_asked_questions_answer1']) 
                                    ? trim($_POST['frequently_asked_questions_answer1']) 
                                    : $settings['frequently_asked_questions_answer1'];
        $newFaqAnswer2        = isset($_POST['frequently_asked_questions_answer2']) 
                                    ? trim($_POST['frequently_asked_questions_answer2']) 
                                    : $settings['frequently_asked_questions_answer2'];
        $newFaqAnswer3        = isset($_POST['frequently_asked_questions_answer3']) 
                                    ? trim($_POST['frequently_asked_questions_answer3']) 
                                    : $settings['frequently_asked_questions_answer3'];
        ...
        // 2) LOGO UPLOAD (Saves as "logo.png" in this current PHP file's directory)
        if (!empty($_FILES['logo_upload']['name']) && $_FILES['logo_upload']['error'] === UPLOAD_ERR_OK) {
            $tmpName   = $_FILES['logo_upload']['tmp_name'];
            $localFile = __DIR__ . '/logo.png'; // Save as "logo.png" in current directory
    
            // Move uploaded file directly (no resizing)
            move_uploaded_file($tmpName, $localFile);
    
            // Optionally update DB to say "logo.png"
            $updateLogo = $conn->prepare("UPDATE tbl_appconfig SET value = 'logo.png' WHERE setting = 'logo_file'");
            $updateLogo->execute();
        }
    
        // 3) ADVERT UPLOAD (Saves as "advert.png" in this current PHP file's directory)
        if (!empty($_FILES['advert_upload']['name']) && $_FILES['advert_upload']['error'] === UPLOAD_ERR_OK) {
            $tmpName   = $_FILES['advert_upload']['tmp_name'];
            $localFile = __DIR__ . '/advert.png'; // Save as "advert.png" in current directory
    
            // Move uploaded file directly (no resizing)
            move_uploaded_file($tmpName, $localFile);
    
            // Optionally update DB to say "advert.png"
            $updateAdvert = $conn->prepare("UPDATE tbl_appconfig SET value = 'advert.png' WHERE setting = 'advert_file'");
            $updateAdvert->execute();
        }
    
        // 4) ADVERT SETTINGS (enable/disable + position)
        $newEnableAdvert   = isset($_POST['enable_advert'])    ? $_POST['enable_advert']    : 'disable'; // default
        $advertPosition    = isset($_POST['advert_position'])  ? $_POST['advert_position']  : 'bottom';
    
        // 5) TESTIMONIALS (enable/disable)
        $enableTestimonials = isset($_POST['enable_testimonials']) ? $_POST['enable_testimonials'] : 'disable';
    
        // 6) FAQ SECTION (enable/disable)
        $newEnableFaq = isset($_POST['enable_faq']) ? $_POST['enable_faq'] : '';
    
        // -- UPDATE DB for newly introduced toggles --
    
        // enable_advert
        $stmt = $conn->prepare("UPDATE tbl_appconfig SET value = :val WHERE setting = 'enable_advert'");
        $stmt->execute([':val' => $newEnableAdvert]);
    
        // advert_position
        $stmt = $conn->prepare("UPDATE tbl_appconfig SET value = :val WHERE setting = 'advert_position'");
        $stmt->execute([':val' => $advertPosition]);
    
        // enable_testimonials
        $stmt = $conn->prepare("UPDATE tbl_appconfig SET value = :val WHERE setting = 'enable_testimonials'");
        $stmt->execute([':val' => $enableTestimonials]);
    
        // enable_faq
        $stmt = $conn->prepare("UPDATE tbl_appconfig SET value = :val WHERE setting = 'enable_faq'");
        $stmt->execute([':val' => $newEnableFaq]);
    
        // -- UPDATE DB for existing fields (title, color scheme, free trial, FAQ headlines/answers) --
...
function getAppConfigValue(PDO $conn, $setting) {
    $stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = :setting");
    $stmt->execute([':setting' => $setting]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    // Return trimmed value or empty string if row not found
    return $row ? trim($row['value']) : '';
}



// 1) Grab all settings from DB, for example:
    $enableAdvert    = getAppConfigValue($conn, 'enable_advert');   // 'enable' or 'disable'
    $advertFile      = getAppConfigValue($conn, 'advert_file');     // e.g. 'advert.png'
    $advertPosition  = getAppConfigValue($conn, 'advert_position'); // 'top' or 'bottom'
    $enableFaq       = getAppConfigValue($conn, 'enable_faq');      // 'enable' or 'disable'
    $enableTestimonials = getAppConfigValue($conn, 'enable_testimonials'); 
    // ...and so on for whatever settings you need
         ...
         if ($enableAdvert === 'enable'
         && $advertPosition === 'top'
     ) {
         $htmlContent .= "    <!-- Top Advert Banner -->\n";
         $htmlContent .= "    <div class=\"flex justify-center items-center mb-6\">\n"; // Centering container
         $htmlContent .= "        <img src=\"advert.png\" alt=\"Advert Banner\" class=\"w-full max-w-2xl object-cover\" style=\"height: auto;\" />\n"; // Centered image
         $htmlContent .= "    </div>\n";
     }
     
         
         // Then show the heading/description as usual
         $htmlContent .= "                <h2 class=\"text-3xl font-extrabold text-gray-900 mb-6\">"
...
if ($enableTestimonials === 'enable') {  
         ...
         $htmlContent .= "                <img class=\"w-full h-48 object-cover object-center\" src=\"https://freeispradius.net/assets/img/testimonials/testimonials-3.jpg\" alt=\"Testimonial from Otieno Peter\">\n";
         ...
         // Testimonial 2
         $htmlContent .= "            <div class=\"bg-white rounded-lg shadow-md overflow-hidden\">\n";
         $htmlContent .= "                <img class=\"w-full h-48 object-cover object-center\" src=\"https://freeispradius.net/assets/img/testimonials/testimonials-2.jpg\" alt=\"Testimonial from Kiveu\">\n";
         $htmlContent .= "                <div class=\"p-4\">\n";
         ...
         // Testimonial 3
         $htmlContent .= "            <div class=\"bg-white rounded-lg shadow-md overflow-hidden\">\n";
         $htmlContent .= "                <img class=\"w-full h-48 object-cover object-center\" src=\"https://freeispradius.net/assets/img/testimonials/testimonials-1.jpg\" alt=\"Testimonial from Anonymous User\">\n";
         $htmlContent .= "                <div class=\"p-4\">\n";
...
if ($enableFaq === 'enable') {  
// Example: if $enableFaq === 'enable', show the FAQ section.
    ...
    $htmlContent .= "                <dt class=\"p-4 cursor-pointer text-lg leading-6 font-medium text-gray-900\" onclick=\"toggleFAQ('faq1')\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_headline1']) 
                 . "</dt>\n";
    $htmlContent .= "                <dd id=\"faq1\" class=\"p-4 hidden text-base text-gray-500\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_answer1']) 
                 . "</dd>\n";
    $htmlContent .= "            </div>\n";

    // FAQ 2
    $htmlContent .= "            <div class=\"bg-white rounded-lg shadow-md\">\n";
    $htmlContent .= "                <dt class=\"p-4 cursor-pointer text-lg leading-6 font-medium text-gray-900\" onclick=\"toggleFAQ('faq2')\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_headline2']) 
                 . "</dt>\n";
    $htmlContent .= "                <dd id=\"faq2\" class=\"p-4 hidden text-base text-gray-500\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_answer2']) 
                 . "</dd>\n";
    $htmlContent .= "            </div>\n";

    // FAQ 3
    $htmlContent .= "            <div class=\"bg-white rounded-lg shadow-md\">\n";
    $htmlContent .= "                <dt class=\"p-4 cursor-pointer text-lg leading-6 font-medium text-gray-900\" onclick=\"toggleFAQ('faq3')\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_headline3']) 
                 . "</dt>\n";
    $htmlContent .= "                <dd id=\"faq3\" class=\"p-4 hidden text-base text-gray-500\">" 
                 . htmlspecialchars($settings['frequently_asked_questions_answer3']) 
                 . "</dd>\n";
    $htmlContent .= "            </div>\n";
...
if ($ftp && ftp_login($ftp, $mikrotik_user, $mikrotik_pass)) {
    ...
    if (ftp_put($ftp, $remoteFile, $localFile /* local path to login.html */, FTP_BINARY)) {
    ...
    if (ftp_put($ftp, $remoteFileFlash, $localFile /* local path to login.html */, FTP_BINARY)) {
    ...
    // ----------------------
    // 2) UPLOAD rlogin.html
    // ----------------------
    $remoteFileRlogin = 'hotspot/rlogin.html';
    if (ftp_put($ftp, $remoteFileRlogin, $localFileRlogin /* local path to rlogin.html */, FTP_BINARY)) {
    ...
    if (ftp_put($ftp, $remoteFileFlashRlogin, $localFileRlogin /* local path to rlogin.html */, FTP_BINARY)) {
    ...
    // 3) UPLOAD logo.png
    // ----------------------
    $localFileLogo = __DIR__ . '/logo.png';  // e.g. "C:\path\to\this\script\logo.png"
    if (file_exists($localFileLogo)) {
        $remoteFileLogo = 'hotspot/logo.png';
        if (ftp_put($ftp, $remoteFileLogo, $localFileLogo, FTP_BINARY)) {
            $logMessages[] = "File uploaded successfully to '$remoteFileLogo'.";
        } else {
            $logMessages[] = "Failed to upload the file to '$remoteFileLogo'.";
        }

        $remoteFileFlashLogo = 'flash/hotspot/logo.png';
        if (ftp_put($ftp, $remoteFileFlashLogo, $localFileLogo, FTP_BINARY)) {
            $logMessages[] = "File uploaded successfully to '$remoteFileFlashLogo'.";
        } else {
            $logMessages[] = "Failed to upload the file to '$remoteFileFlashLogo'.";
        }
    } else {
        $logMessages[] = "logo.png does not exist locally, skipping upload.";
    }

    // -----------------------
    // 4) UPLOAD advert.png
    // -----------------------
    $localFileAdvert = __DIR__ . '/advert.png';  // e.g. "C:\path\to\this\script\advert.png"
    if (file_exists($localFileAdvert)) {
        $remoteFileAdvert = 'hotspot/advert.png';
        if (ftp_put($ftp, $remoteFileAdvert, $localFileAdvert, FTP_BINARY)) {
            $logMessages[] = "File uploaded successfully to '$remoteFileAdvert'.";
        } else {
            $logMessages[] = "Failed to upload the file to '$remoteFileAdvert'.";
        }

        $remoteFileFlashAdvert = 'flash/hotspot/advert.png';
        if (ftp_put($ftp, $remoteFileFlashAdvert, $localFileAdvert, FTP_BINARY)) {
            $logMessages[] = "File uploaded successfully to '$remoteFileFlashAdvert'.";
        } else {
            $logMessages[] = "Failed to upload the file to '$remoteFileFlashAdvert'.";
        }
    } else {
        $logMessages[] = "advert.png does not exist locally, skipping upload.";
    }
...
$freeTrialEnabled = ($result && $result['value'] === 'enable');
    ...
    // Example of reading stored "logo_file" from DB:
    $stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'logo_file'");
    $stmt->execute();
    $logoFile = $stmt->fetch(PDO::FETCH_ASSOC);
    $logoFile = $logoFile ? $logoFile['value'] : 'logo.png';

    // Example of reading stored "logo_file" from DB:
    $stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'advert_file'");
    $stmt->execute();
    $advertFile = $stmt->fetch(PDO::FETCH_ASSOC);
    $advertFile = $advertFile ? $advertFile['value'] : 'advert.png';



// Try reading from tbl_appconfig
$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'enable_advert'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);

if ($res) {
    $enableAdvert = $res['value'];
}

// Assign to Smarty
$ui->assign('enable_advert', $enableAdvert);

// Default if not found


// Attempt to get from tbl_appconfig
$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'enable_faq'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);
...
// Assign to Smarty
$ui->assign('enable_faq', $enableFaq);



// 1) Read enable_advert from DB

$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'enable_advert'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);
if ($res) {
    $enableAdvert = $res['value']; // 'enable' or 'disable'
}
$ui->assign('enable_advert', $enableAdvert);


$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'enable_testimonials'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);
if ($res) {
    $enableTestimonials = $res['value']; // 'enable' or 'disable'
}
$ui->assign('enable_testimonials', $enableTestimonials);

// 3) Read enable_faq from DB

$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'enable_faq'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);
if ($res) {
    $enableFaq = $res['value']; // 'enable' or 'disable'
}
$ui->assign('enable_faq', $enableFaq);

// 4) Read advert_position from DB
$advertPosition = 'bottom'; // default
$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'advert_position'");
$stmt->execute();
$res = $stmt->fetch(PDO::FETCH_ASSOC);
if ($res) {
    $advertPosition = $res['value']; // 'top' or 'bottom'
}
$ui->assign('advert_position', $advertPosition);
...
// Retrieve advert_position from DB
$stmt = $conn->prepare("SELECT value FROM tbl_appconfig WHERE setting = 'advert_position'");
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_ASSOC);

// If not found, default to 'bottom'
$advertPosition = $result ? $result['value'] : 'bottom';

// Pass it on to the Smarty template
$ui->assign('advert_position', $advertPosition);
