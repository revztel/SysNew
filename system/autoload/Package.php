 ...
 if ($p['validity_unit'] == 'Months') {
    ...
    $time = $datetime[1];
                ...
                if ($p['is_radius']) {
                    ...
                    Mikrotik::removeHotspotActiveUser($client, $c['username']);
                    //Mikrotik::removePpoeActive($client, $c['username']);
                    $c['expiration']  = "$date_exp $time";  // set it from your logic
                    Mikrotik::addHotspotUser($client, $p, $c);
                }                
            ...
            // Query for midnight_expiry
$midnightExpiryConfig = ORM::for_table('tbl_appconfig')
->where('setting', 'midnight_expiry')
->find_one();
...
// Evaluate
$midnightExpiry = (
$midnightExpiryConfig
&& $midnightExpiryConfig['value'] == 'yes'
) ? true : false;
...
// Log final boolean
            ...
            // If user already has a plan row in tbl_user_recharges
            if ($b) {
        
                // Calculate new expiration from *current* date/time
                if ($p['validity_unit'] == 'Months') {
                    $date_exp = date("Y-m-d", strtotime('+' . $p['validity'] . ' month'));
                } 
                else if ($p['validity_unit'] == 'Days') {
                ...
                // If midnight expiry is enabled, force time to 00:00:00
                if ($midnightExpiry) {
                    $time = '00:00:00';
                }
        
                // Remove existing PPPoE user from Mikrotik
                $client = Mikrotik::getClient($mikrotik['ip_address'], $mikrotik['username'], $mikrotik['password']);
                Mikrotik::removePpoeUser($client, $c['username']);
                Mikrotik::removePpoeActive($client, $c['username']);

                $c['expiration']  = "$date_exp $time";  // set it from your logic
                // Add the new PPPoE user
                ...
                // No existing plan row, so create one
                ...
                // Create the new record in tbl_user_recharges
                $d = ORM::for_table('tbl_user_recharges')->create();
                $d->customer_id     = $id_customer;
                $d->username        = $c['username'];
                $d->plan_id         = $plan_id;
...
&& $midnightExpiryConfig['value'] == 'yes'
            ...
            if ($b) {
                // If plan is radius-based or not
                if ($p['is_radius']) {
                    Radius::customerAddPlan($c, $p, "$date_exp $time");
                } else {
                    $client = Mikrotik::getClient($mikrotik['ip_address'], $mikrotik['username'], $mikrotik['password']);
                ...
                // If it's the same internet plan and currently active, we extend the expiration
                if ($b['namebp'] == $p['name_plan'] && $b['status'] == 'on') {
                    // Calculate the new expiration date based on the current date and validity period
                    if ($p['validity_unit'] == 'Months') {
                        $date_exp = date("Y-m-d", strtotime('+' . $p['validity'] . ' month'));
                ...
                // Update existing record in tbl_user_recharges
                $b->customer_id    = $id_customer;
                $b->username       = $c['username'];
                $b->plan_id        = $plan_id;
                $b->namebp         = $p['name_plan'];
