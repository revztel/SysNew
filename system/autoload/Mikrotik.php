...

{
            ...
            if ($plan['limit_type'] == "Time_Limit") {
                ...
                $client->sendSync(
                    $addRequest
                        ...
                        ->setArgument('comment', 'Exp: ' . $customer['expiration']) // Only expiry
                ...
                $client->sendSync(
                    $addRequest
                        ...
                        ->setArgument('profile', $plan['name_plan'])
                        ->setArgument('password', $customer['password'])
                        ->setArgument('comment', 'Exp: ' . $customer['expiration'])
                        ->setArgument('email', $customer['email'])
    ...
    public static function addPpoeUser($client, $plan, $customer)
    {
        ...
        // Full Name | Phone Number | Exp: ...
        $commentText = $customer['fullname'] 
            . ' | ' . $customer['phonenumber'] 
            . ' | Exp: ' . $customer['expiration'];
        ...
        $client->sendSync(
            $addRequest
                ...
                ->setArgument('profile', $plan['name_plan'])
                ->setArgument('comment', $commentText)  // changed here
                ->setArgument('password', $pass)
        );
    }
...
public static function addStaticUser($client, $plan, $customer)
{
    ...
    // We'll build a comment that has name, phone, and expiry
    // e.g. "John Doe | 1234567890 | Exp: 2025-01-15 00:00:00"
    $commentForIPs = $customer['fullname']
        . ' | ' . $customer['phonenumber']
        . ' | Exp: ' . $customer['expiration'];

    // Define names for the queue and address list entries
    $queueName = !empty($customer['ip_address']) 
        ? 'Queue-' . $customer['username'] 
        : 'Queue [username] IP not found';
    $addressListName = !empty($customer['ip_address']) 
        ? $customer['username'] 
        : 'IP not found';

    // 1) Add the IP to the RouterOS address list named 'allowed'
    // Use the new comment with name, phone, expiry
    $addAddressListRequest = new RouterOS\Request('/ip/firewall/address-list/add');
    $client->sendSync(
        $addAddressListRequest
            ->setArgument('list', 'allowed')
            ->setArgument('address', $ipAddress)
            ->setArgument('comment', $commentForIPs)  // changed here
    );

    // Retrieve bandwidth details from the database
    $b = ORM::for_table('tbl_bandwidth')->where('id', $plan['id_bw'])->find_one();

    // Determine rate units for download
    ...
    // Determine rate units for upload
    ...
    // Construct the max-limit string (upload/download)
    $maxLimit = $rateUp . "/" . $rateDown;

    // Also define a comment for the queue that includes name, phone, expiry
    $commentForQueue = $customer['fullname']
        . ' | ' . $customer['phonenumber']
        . ' | Exp: ' . $customer['expiration'];

    // Check if all burst fields are present
    if (
        !empty($b['burst_limit_up']) && !empty($b['burst_limit_down']) &&
        !empty($b['burst_threshold_up']) && !empty($b['burst_threshold_down']) &&
        !empty($b['burst_time'])
    ) {
        // Handle burst-limit for upload
        ...
        // Handle burst-limit for download
        ...
        // Handle burst-threshold for upload
        ...
        // Handle burst-threshold for download
        ...
        // Set up a simple queue with burst parameters
        $addQueueRequest = new RouterOS\Request('/queue/simple/add');
        $client->sendSync(
            $addQueueRequest
                ...
                ->setArgument('comment', $commentForQueue)  // changed here
        );
    } 
    else {
        // No burst
