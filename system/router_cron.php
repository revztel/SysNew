...
function updateURLIfContains($settingName, $searchString, $replaceString) {
    // Check if the setting exists and contains the search string
    $existingSetting = ORM::for_table('tbl_appconfig')
        ->where('setting', $settingName)
        ->find_one();

    if ($existingSetting && strpos($existingSetting->value, $searchString) !== false) {
        // Replace the search string with the replace string
        $existingSetting->value = str_replace($searchString, $replaceString, $existingSetting->value);
        $existingSetting->save();
        echo "Setting '{$settingName}' updated from '{$searchString}' to '{$replaceString}'.\n";
    } else {
        echo "Setting '{$settingName}' does not need updating.\n";
    }
}

// Update 'sms_url' and 'wa_url' settings if they contain 'freeispradius'
updateURLIfContains('sms_url', 'freeispradius', 'ispledger');
updateURLIfContains('wa_url', 'freeispradius', 'ispledger');
