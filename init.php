...
if (file_exists($lan_file)) {
    ...
    $_L['author'] = 'Auto Generated by FreeIspRadius';
    $_SESSION['Lang'] = $_L;
...
function _admin($login = true)
{
    ...
    if (Admin::getID()) {
        ...
        if ($active_session) {
            // Update last_activity and ip_address
            $active_session->last_activity = date('Y-m-d H:i:s');
            $active_session->ip_address = $ip_address;
            $active_session->save();
        } else {
            // Create a new session record
            $new_session = ORM::for_table('tbl_active_sessions')->create();
            $new_session->session_id = $session_id;
            $new_session->user_id = $user_id;
            $new_session->ip_address = $ip_address;
            $new_session->last_activity = date('Y-m-d H:i:s');
            $new_session->save();
        }
        ...
        if ($login) {
            r2(U . 'login'); // Redirect to login page
        } else {
            return false; // Return false if not logging in
        }
    }
}
